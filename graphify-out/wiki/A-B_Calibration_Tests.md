# A/B Calibration Tests

> 45 nodes

## Key Concepts

- **contracts.py** (51 connections) — `app/engine/contracts.py`
- **DecisionReason** (33 connections) — `app/engine/contracts.py`
- **opportunities.py** (24 connections) — `app/engine/opportunities.py`
- **candidate()** (11 connections) — `app/engine/opportunities.py`
- **StrEnum** (10 connections) — `app/engine/contracts.py`
- **OpportunityState** (6 connections) — `app/engine/contracts.py`
- **market_context.py** (6 connections) — `app/engine/market_context.py`
- **recommend_entry()** (6 connections) — `app/engine/opportunities.py`
- **list_candidates()** (6 connections) — `app/engine/opportunities.py`
- **OpportunityCandidate** (4 connections) — `app/engine/contracts.py`
- **snapshot()** (4 connections) — `app/engine/market_context.py`
- **_decimal()** (4 connections) — `app/engine/opportunities.py`
- **top_down()** (4 connections) — `app/engine/opportunities.py`
- **_quality()** (4 connections) — `app/engine/opportunities.py`
- **rank()** (4 connections) — `app/engine/opportunities.py`
- **test_broker_factory.py** (4 connections) — `app/tests/test_broker_factory.py`
- **TopDownState** (3 connections) — `app/engine/contracts.py`
- **MarketContextSnapshot** (3 connections) — `app/engine/contracts.py`
- **TopDownDecision** (3 connections) — `app/engine/contracts.py`
- **FactorGrade** (3 connections) — `app/engine/contracts.py`
- **TradeSetup** (3 connections) — `app/engine/contracts.py`
- **EntryRecommendation** (3 connections) — `app/engine/contracts.py`
- **lifecycle()** (3 connections) — `app/engine/opportunities.py`
- **_ungraded()** (3 connections) — `app/engine/opportunities.py`
- **_private_custody_by_setup()** (3 connections) — `app/engine/opportunities.py`
- *... and 20 more nodes in this community*

## Relationships

- [Chart Vendor Line Renderers](Chart_Vendor_Line_Renderers.md) (22 shared connections)
- [Cycle Detection Engine](Cycle_Detection_Engine.md) (9 shared connections)
- [Chart Vendor Series](Chart_Vendor_Series.md) (9 shared connections)
- [Shared Pipeline Loop Tests](Shared_Pipeline_Loop_Tests.md) (6 shared connections)
- [Market Data Importer](Market_Data_Importer.md) (3 shared connections)
- [ni](ni.md) (2 shared connections)
- [Chart Vendor Marker Rendering](Chart_Vendor_Marker_Rendering.md) (2 shared connections)
- [One Source of Truth JS Tests](One_Source_of_Truth_JS_Tests.md) (2 shared connections)
- [test_phemex_private.py](test_phemex_private.py.md) (2 shared connections)
- [Chart Vendor Renderer Base](Chart_Vendor_Renderer_Base.md) (2 shared connections)
- [Notification Delivery](Notification_Delivery.md) (1 shared connections)
- [Execution Simulator & Risk](Execution_Simulator_%26_Risk.md) (1 shared connections)

## Source Files

- `app/engine/contracts.py`
- `app/engine/market_context.py`
- `app/engine/opportunities.py`
- `app/tests/test_broker_factory.py`

## Audit Trail

- EXTRACTED: 210 (91%)
- INFERRED: 22 (9%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*