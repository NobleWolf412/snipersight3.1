# Trade Surface JS Tests

> 13 nodes

## Key Concepts

- **test_trade_surfaces.js** (14 connections) — `app/tests/test_trade_surfaces.js`
- **S()** (2 connections) — `app/tests/test_trade_surfaces.js`
- **ok()** (2 connections) — `app/tests/test_trade_surfaces.js`
- **armSrc()** (2 connections) — `app/tests/test_trade_surfaces.js`
- **fs** (1 connections) — `app/tests/test_trade_surfaces.js`
- **path** (1 connections) — `app/tests/test_trade_surfaces.js`
- **assert** (1 connections) — `app/tests/test_trade_surfaces.js`
- **HTML** (1 connections) — `app/tests/test_trade_surfaces.js`
- **CSS** (1 connections) — `app/tests/test_trade_surfaces.js`
- **CHART** (1 connections) — `app/tests/test_trade_surfaces.js`
- **FUNNEL** (1 connections) — `app/tests/test_trade_surfaces.js`
- **TRACER** (1 connections) — `app/tests/test_trade_surfaces.js`
- **setDevBody** (1 connections) — `app/tests/test_trade_surfaces.js`

## Relationships

- [test_one_source_of_truth.js](test_one_source_of_truth.js.md) (1 shared connections)

## Source Files

- `app/tests/test_trade_surfaces.js`

## Audit Trail

- EXTRACTED: 27 (96%)
- INFERRED: 1 (4%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*