# execsim.py

> 24 nodes

## Key Concepts

- **zi** (24 connections) — `app/static/lightweight-charts.js`
- **.ol()** (10 connections) — `app/static/lightweight-charts.js`
- **.An()** (9 connections) — `app/static/lightweight-charts.js`
- **.hl()** (8 connections) — `app/static/lightweight-charts.js`
- **.Xs()** (7 connections) — `app/static/lightweight-charts.js`
- **.el()** (7 connections) — `app/static/lightweight-charts.js`
- **.rl()** (6 connections) — `app/static/lightweight-charts.js`
- **.Ei()** (6 connections) — `app/static/lightweight-charts.js`
- **.sl()** (5 connections) — `app/static/lightweight-charts.js`
- **.dl()** (5 connections) — `app/static/lightweight-charts.js`
- **.Kh()** (4 connections) — `app/static/lightweight-charts.js`
- **.al()** (4 connections) — `app/static/lightweight-charts.js`
- **.ul()** (4 connections) — `app/static/lightweight-charts.js`
- **.cl()** (4 connections) — `app/static/lightweight-charts.js`
- **.fl()** (4 connections) — `app/static/lightweight-charts.js`
- **._l()** (3 connections) — `app/static/lightweight-charts.js`
- **.nl()** (2 connections) — `app/static/lightweight-charts.js`
- **.Ni()** (2 connections) — `app/static/lightweight-charts.js`
- **.Xr()** (2 connections) — `app/static/lightweight-charts.js`
- **.ll()** (2 connections) — `app/static/lightweight-charts.js`
- **.vl()** (2 connections) — `app/static/lightweight-charts.js`
- **.constructor()** (1 connections) — `app/static/lightweight-charts.js`
- **.ie()** (1 connections) — `app/static/lightweight-charts.js`
- **.J()** (1 connections) — `app/static/lightweight-charts.js`

## Relationships

- [Chart Vendor Coordinates](Chart_Vendor_Coordinates.md) (6 shared connections)
- [Chart Vendor Watermark](Chart_Vendor_Watermark.md) (3 shared connections)
- [Chart Vendor Series Views](Chart_Vendor_Series_Views.md) (2 shared connections)
- [Chart Vendor Hit Testing](Chart_Vendor_Hit_Testing.md) (2 shared connections)
- [Chart Vendor Layout](Chart_Vendor_Layout.md) (2 shared connections)
- [Chart Vendor API](Chart_Vendor_API.md) (1 shared connections)
- [Glossary UI](Glossary_UI.md) (1 shared connections)
- [Chart Vendor Chart API](Chart_Vendor_Chart_API.md) (1 shared connections)
- [Chart Vendor Rendering](Chart_Vendor_Rendering.md) (1 shared connections)
- [Chart Vendor Panes](Chart_Vendor_Panes.md) (1 shared connections)
- [Chart Vendor Price Scale Formatting](Chart_Vendor_Price_Scale_Formatting.md) (1 shared connections)
- [makeRail](makeRail.md) (1 shared connections)

## Source Files

- `app/static/lightweight-charts.js`

## Audit Trail

- EXTRACTED: 121 (98%)
- INFERRED: 2 (2%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*