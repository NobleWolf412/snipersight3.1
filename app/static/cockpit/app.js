...
async function drawTradeHistory(dialog,trade,comparison){
  ...
  const study= comparison?.items?.[0];
  ...
  try{
    ...
    if(study&&study.state!=='EXCLUDED')for(const [rule,color] of [['COST_COVER','#d7b2ff'],['STRUCTURE','#f0c37e']]){
      ...
      const trace=historyChart.addLineSeries({priceFormat,color,lineWidth:2,lineType:1,lineStyle:rule==='COST_COVER'?2:0,lastValueVisible:false,priceLineVisible:false,title:rule==='COST_COVER'?'Cost-cover stop (simulated)':'Swing stop (simulated)'});
...
function bindDiagnoses(){document.querySelectorAll('[data-diagnosis]').forEach(button=>button.onclick=async()=>{
  try{
    ...
    dialog.querySelector('.trade-result-hero').insertAdjacentHTML('afterend',excursionPanel(trade.excursion)+stopStudyPanel(data.stop_comparison,true));
    $('#content').append(dialog);button.focus();dialog.showModal();void drawTradeHistory(dialog,trade,data.stop_comparison);dialog.addEventListener('close',()=>{dialog.remove();if(button.isConnected)button.focus();},{once:true});
