...
function planView(ev){
  ...
  return `<div class="evidence-block"><h4>Checks the bot required — all passed</h4><ul class="plan-checks">${checks}</ul>${econ}</div>`;
}
...
function nearestAgainst(row){const p=row.trade_evidence?.higher_timeframe?.pools?.against||[];return p.length?Math.min(...p.map(x=>Number(x.distance_pct))):1e9;}
