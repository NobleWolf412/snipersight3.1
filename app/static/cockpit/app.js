...
function signalResearchPanel(report){
  if(!report)return `<section class="card section-gap"><h2>Signal research — which readings have earned trust?</h2><p>Data unavailable. No missing reading is shown as zero or neutral.</p></section>`;
  const cards=(report.rows||[]).map(row=>{const p=row.progress||{},interval=row.ci_lo==null?'Not gradeable':`[${Number(row.ci_lo).toFixed(3)}, ${Number(row.ci_hi).toFixed(3)}]R`;return `<article class="signal-evidence-card"><div class="card-head"><div><div class="eyebrow">${esc(row.verdict)}</div><h3>${esc(row.label)}</h3></div>${badge('Used in trading: No')}</div><p>${esc(row.hypothesis)}</p>${facts([['Exposed',`${row.exposed_count} trades · ${row.exposed_symbol_clusters} symbols`],['Control',`${row.control_count} trades · ${row.control_symbol_clusters} symbols`],['Progress',`Exposed ${p.exposed_trades}/${p.trades_required_each} trades, ${p.exposed_symbols}/${p.symbols_required_each} symbols · Control ${p.control_trades}/${p.trades_required_each} trades, ${p.control_symbols}/${p.symbols_required_each} symbols`],['Coverage / missing',`${Math.round(Number(row.coverage||0)*100)}% / ${Math.round(Number(row.missing_rate||0)*100)}%`],['Net-R uplift after costs',row.uplift_r==null?'Not gradeable':`${Number(row.uplift_r).toFixed(3)}R`],['95% interval',interval],['Stability',row.sample_ok?(row.stable?'Stable across the time split':'Not stable'):'Not gradeable'],['Corrected significance',row.q_value==null?'Not gradeable':Number(row.q_value).toFixed(3)],['Collection start',row.collection_start?date(row.collection_start):'Not started'],['Detector version',row.detector_version]])}<details><summary>Other patterns observed</summary><ul>${(row.other_patterns_observed||[]).map(item=>`<li>${esc(item.label)} <small>${item.status==='EXPLORATORY_UNCOLLECTED'?'Planned exploratory pattern — no observations collected.':'Exploratory — cannot be Proven useful'}</small></li>`).join('')||'<li>None reported</li>'}</ul></details></article>`;}).join('');
  return `<section class="card signal-research section-gap"><div class="eyebrow">SIGNALS · RESEARCH ONLY</div><h2>Signal research — which readings have earned trust?</h2><p class="signal-research-verdict">${esc(report.verdict||'Data unavailable')}</p><p class="ticket-note">Used in trading: No. Each side needs ${esc(report.minimums?.closed_trades_each||30)} completed trades and ${esc(report.minimums?.symbol_clusters_each||8)} symbol clusters.</p><div class="signal-evidence-grid">${cards}</div></section>`;
}
