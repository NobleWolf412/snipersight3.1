...
const signalMap = window.SSSignalMap;
...
let context = null, page = 'home', selectedSetup = null, chart = null, resize = null, noticeTimer, chartLayersCleanup = null;

// Read-only chart-state seam used by rendered integration checks. The classic
// workspace exposes the same shape through SSChartResearchHost.
window.SSCockpitChartHost=Object.freeze({chart:()=>chart});
...
// A study restarted under new rules keeps its earlier run apart rather than mixing the two.
function restartNote(x) { return x?.previous ? `<p class="source-detail">Restarted fresh under updated rules. The earlier run, started ${esc(date(x.previous.started_at))}, is kept separately and not counted here.</p>` : ''; }
...
function disposeChart(){if(chartLayersCleanup)chartLayersCleanup();chartLayersCleanup=null;if(resize)resize.disconnect();resize=null;if(chart)chart.remove();chart=null;$('#opportunity-level-key')?.remove();}
...
function confirmationView(g,{compact=false}={}){
  if(!g)return '';
  if(!g.available)return `<p class="ticket-note">${esc(g.unavailable_reason||g.confirmation)}</p>${facts([['Area lower edge',g.zone_bottom||'Not recorded'],['Area upper edge',g.zone_top||'Not recorded']])}<p>${esc(g.skip_if)}</p>`;
  const chart=g.mini_chart;
  const timer=g.next_close_at?`<span data-confirm-countdown="${esc(g.next_close_at-g.as_of)}" data-clock-start="${Date.now()}">Next candle closes ${esc(date(g.next_close_at))}</span>`:'';
  return `<div class="confirmation-guide"><div class="confirmation-idea"><span class="eyebrow">${esc(g.intended_trade)}</span><p>${esc(g.market_context)}</p><small>Structure when this setup was recorded</small></div><div class="confirmation-chart">${chart?`<svg viewBox="0 0 310 154" role="img" aria-label="Recent completed candles, watched area, confirmation edge and last recorded close"><rect class="watched-zone" x="8" width="294" y="${esc(chart.zone_y)}" height="${esc(chart.zone_height)}"/>${chart.bars.map(b=>`<g class="mini-candle ${b.rising?'up':'down'}"><line x1="${esc(b.x)}" x2="${esc(b.x)}" y1="${esc(b.high)}" y2="${esc(b.low)}"/><rect x="${esc(b.x-4)}" width="8" y="${esc(b.body_top)}" height="${esc(b.body_height)}"/></g>`).join('')}<line class="confirmation-edge" x1="8" x2="302" y1="${esc(chart.boundary_y)}" y2="${esc(chart.boundary_y)}"/><line class="recorded-price" x1="8" x2="302" y1="${esc(chart.last_y)}" y2="${esc(chart.last_y)}"/></svg>`:'<p class="ticket-note">No completed candles available for this chart.</p>'}<div class="confirmation-key"><span class="zone-key">Area <strong>${esc(g.zone_bottom)}–${esc(g.zone_top)}</strong></span><span class="trigger-key">Close ${g.direction==='LONG'?'above':'below'} <strong>${esc(g.confirmation_boundary)}</strong></span><span class="price-key">Last close <strong>${esc(g.last_price??'Unavailable')}</strong></span></div><p class="source-detail">${esc(g.timeframe)} completed candles · ${esc(date(g.last_closed_at))}${g.data_stale?' · Price data delayed':''}. Not a live quote.</p></div><div class="confirmation-trigger"><h4>${g.state==='FORMING'?'After price reaches the area':'What confirms this setup'}</h4><p>All three conditions must hold on the same completed candle:</p><ol>${g.conditions.map(c=>`<li>${esc(c)}</li>`).join('')}</ol></div>${g.steps?`<div class="confirmation-progress"><div class="candle-steps" aria-label="${esc(g.completed_followup_bars)} of ${esc(g.max_followup_bars)} follow-up candles recorded">${g.steps.map((s,i)=>`<div class="${s.complete?'complete':''}"><span aria-hidden="true">${s.complete?'✓':i+1}</span><small>Candle ${i+1}</small></div>`).join('')}</div><strong>${esc(g.completed_followup_bars)} of ${esc(g.max_followup_bars)} follow-up candles recorded</strong><p class="ticket-note">The touch candle is checked first, then up to ${esc(g.max_followup_bars)} more. These steps count recorded candles, not passed conditions.</p><p class="confirmation-clock">${g.window_ended?'Confirmation window ended · awaiting the recorded outcome':timer}${g.missing_candles?' · Waiting for missing candle data':''}</p><small>Deadline ${esc(date(g.confirmation_deadline))}</small></div>`:g.state==='FORMING'?'<p class="ticket-note">The confirmation countdown starts when price touches the area.</p>':'<p class="ticket-note">This setup has left the confirmation stage.</p>'}<${compact?'details':'div'} class="confirmation-outcomes">${compact?'<summary>What cancels it?</summary>':'<h4>What cancels it?</h4>'}<p>${esc(g.skip_if)}</p><h4>If time runs out</h4><p>${esc(g.expiry_action)}</p><p class="ticket-note">${esc(g.after_confirmation)}</p></${compact?'details':'div'}></div>`;
}
function updateConfirmationClocks(){
  document.querySelectorAll('[data-confirm-countdown]').forEach(node=>{
    const seconds=Math.max(0,Math.ceil(Number(node.dataset.confirmCountdown)-(Date.now()-Number(node.dataset.clockStart))/1000));
    node.textContent=seconds?`Next candle closes in ${Math.floor(seconds/3600)?Math.floor(seconds/3600)+'h ':''}${Math.floor(seconds%3600/60)}m ${seconds%60}s`:'Candle close time reached · refresh for recorded progress';
  });
}
/* TRADE EVIDENCE — what supports and what conflicts with one trade.
   The higher-timeframe picture comes first because it answers "am I standing
   in front of a bigger move". Every value is the server's: nothing here adds,
   counts or ranks factors, and there is no overall score on purpose — no
   factor has been graded yet (setup_guide.confluence_rows). */
const FACTOR_TONE={SUPPORTS:['Supports','good'],CONFLICTS:['Conflicts','bad'],NEUTRAL:['Neutral','neutral'],UNAVAILABLE:['Not available','neutral'],INFO:['Info','neutral']};
const STANCE_TONE={COUNTER_INTO_RUNNING_MOVE:'bad',COUNTER_TREND:'warning',MIXED:'warning',WITH_TREND:'good'};
const STANCE_WORD={COUNTER_INTO_RUNNING_MOVE:'Against a running move',COUNTER_TREND:'Counter-trend',MIXED:'Mixed',WITH_TREND:'With the trend',NO_CLEAR_TREND:'No clear trend',NO_HIGHER_TIMEFRAME:'No higher timeframe',NOT_ENOUGH_HISTORY:'Not enough history'};
function tfName(tf){return tf==='1D'?'Daily':String(tf||'');}
function touches(n){return `${esc(n)} touch${Number(n)===1?'':'es'}`;}
function poolLine(p){return `<span class="pool-line"><strong>${esc(tfName(p.tf))}</strong> ${esc(p.level)} · ${esc(p.distance_pct)}% away · ${esc(p.status)}</span>`;}
function higherTimeframeView(h,direction){
  if(!h)return '<p class="ticket-note">Higher-timeframe context is unavailable for this setup.</p>';
  const st=h.stance||{},tone=STANCE_TONE[st.label]||'neutral';
  const ladder=(h.ladder||[]).map(r=>`<li><strong>${esc(tfName(r.tf))}</strong> ${esc(r.words)}</li>`).join('');
  const short=direction==='SHORT',against=h.pools?.against||[],toward=h.pools?.toward||[];
  const noPools=(h.pools?.measured||[]).length?'None found':'No liquidity records up to daily for this market';
  const lvl=(l,where)=>l?`${esc(l.price)} (${esc(l.distance_pct)}% ${where}, ${touches(l.touches)})`:`none ${where}`;
  const levels=(h.levels||[]).map(l=>`<li><strong>${esc(tfName(l.tf))}</strong> resistance ${lvl(l.resistance,'above')} · support ${lvl(l.support,'below')}</li>`).join('');
  return `<div class="evidence-block htf-${esc(tone)}"><h4>Higher timeframes now ${badge(STANCE_WORD[st.label]||'Unknown',tone)}</h4><p class="stance-sentence">${esc(st.sentence||'')}</p>${st.strategy_note?`<p class="ticket-note">${esc(st.strategy_note)}</p>`:''}${ladder?`<ul class="htf-ladder">${ladder}</ul>`:''}<h4>Liquidity pools</h4><p class="source-detail">Clusters of equal highs or lows where stop orders rest. Nearest on each timeframe, up to daily.</p><dl class="facts"><div><dt>${short?'Above you (against a short)':'Below you (against a long)'}</dt><dd>${against.length?against.map(poolLine).join(''):noPools}</dd></div><div><dt>${short?'Below you (the target side)':'Above you (the target side)'}</dt><dd>${toward.length?toward.map(poolLine).join(''):noPools}</dd></div></dl>${levels?`<h4>Support and resistance</h4><p class="source-detail">Prices touched repeatedly on each timeframe — a different measure from liquidity pools.</p><ul class="htf-ladder">${levels}</ul>`:''}<p class="source-detail">Distances measured from ${esc(h.price)}, the last closed ${esc(h.price_timeframe||'')} candle at ${esc(date(h.price_at))}. Higher timeframes read from completed candles only.</p>${h.price_stale?'<p class="ticket-note">This price is out of date — the market data has not updated for over a candle. Distances may be wrong.</p>':''}</div>`;
}
function confluenceView(ev){
  if(!ev.factors)return `<div class="evidence-block"><h4>Confluence</h4><p class="ticket-note">${esc(ev.confluence_reason||'Confluence is not recorded for this setup.')}</p></div>`;
  const rows=ev.factors.map(f=>{const [word,tone]=FACTOR_TONE[f.state]||['—','neutral'];return `<li class="factor-row"><div><strong>${esc(f.factor)}</strong><small class="source-detail">${esc(f.detail)}</small></div><div class="factor-value">${badge(word,tone)}<span>${esc(f.value)}</span></div></li>`;}).join('');
  const r=ev.recorded_context;
  return `<div class="evidence-block"><h4>Confluence when it confirmed</h4><ul class="factor-list">${rows}</ul>${r&&r.alignment_words?`<p class="source-detail">Whole ladder at confirmation: ${esc(r.alignment_words)}${r.words?` · one timeframe up was ${esc(r.words)}`:''}.</p>`:''}</div>`;
}
function planView(ev){
  if(!ev.required)return '';
  const e=ev.economics||{};
  const checks=ev.required.map(c=>`<li><span aria-hidden="true">✓</span> ${esc(c.check)}${c.value==null?'':(c.check.startsWith('A completed')?` · ${esc(date(c.value))}`:` · ${esc(c.value)}${c.check.startsWith('Risk')?'×':''}`)}</li>`).join('');
  const econ=e.rr_net?`<dl class="facts"><div><dt>Reward / risk before costs</dt><dd>${esc(e.rr_gross)} R</dd></div><div><dt>After estimated costs</dt><dd><strong>${esc(e.rr_net)} R</strong></dd></div><div><dt>Estimated costs</dt><dd>${esc(e.cost_r)} R</dd></div></dl><p class="source-detail">${esc(e.basis)}</p>`:(e.reason?`<p class="ticket-note">${esc(e.reason)}</p>`:'');
  return `<div class="evidence-block"><h4>Plan checks — all cleared</h4><ul class="plan-checks">${checks}</ul>${econ}</div>`;
}
function evidenceView(ev){
  if(!ev)return '';
  return `<section class="trade-evidence" aria-label="Trade evidence"><p class="ticket-note">${esc(ev.note)}</p>${higherTimeframeView(ev.higher_timeframe,ev.direction)}${confluenceView(ev)}${planView(ev)}</section>`;
}
/* The Compare tab. Rows arrive newest first and the operator re-sorts by the
   column that matters; no ordering here is a verdict. Sorting reads server
   values only. */
const COMPARE_SORTS={
  newest:(a,b)=>Number(b.setup.confirmed_at||0)-Number(a.setup.confirmed_at||0),
  net_rr:(a,b)=>Number(b.trade_evidence?.economics?.rr_net??-1e9)-Number(a.trade_evidence?.economics?.rr_net??-1e9),
  pool_against:(a,b)=>nearestAgainst(b)-nearestAgainst(a),
};
function nearestAgainst(row){const pools=row.trade_evidence?.higher_timeframe?.pools||{},p=pools.against||[];if(!(pools.measured||[]).length)return -1;return p.length?Math.min(...p.map(x=>Number(x.distance_pct))):1e9;}
function compareTable(rows){
  if(!rows.length)return empty('No open trade plans to compare','Plans appear here once a setup confirms and while its entry window is open, whether or not the bot took it.');
  return `<div class="setup-list">${rows.map((r,i)=>{const ev=r.trade_evidence||{},e=ev.economics||{};return `<article class="setup-row compare-row" data-state="${esc(r.state)}"><div class="card-head"><div><h3>${esc(r.setup.symbol)} · ${esc(label(r.setup.direction))}</h3><p class="source-detail">${esc(r.setup.timeframe)} · ${esc(label(r.setup.strategy))} · ${e.rr_net?`${esc(e.rr_net)} R after costs`:'costs not estimated'}</p></div>${r.tradeable_by_hand===false?badge('Not tradeable here','bad'):r.tradeable_by_hand==null?badge('Venue unknown','warning'):badge(r.state==='READY'?'Ready to trade':'Bot declined','neutral')}</div>${r.blocked_because?.length?`<p class="source-detail">Why the bot declined: ${esc(r.blocked_because.join(' · '))}</p>`:''}${evidenceView(ev)}${setupTimes(r)}<button class="small" data-setup="${i}">Review trade</button></article>`;}).join('')}</div>`;
}
function opportunitiesTable(rows){if(!rows.length)return empty('No matching setups','Ready trades appear once their prices and trading checks are confirmed. Use Watching to see setups still developing.');return `<div class="setup-list">${rows.map((r,i)=>`<article class="setup-row" data-state="${esc(r.state)}"><div class="card-head"><div><h3>${esc(r.setup.symbol)} · ${esc(label(r.setup.direction))}</h3><p class="source-detail">${esc(r.setup.timeframe)} · ${esc(label(r.setup.strategy))}</p></div>${badge(r.progress_label||(r.actionable?'Ready to trade':'Developing setup'))}</div>${r.confirmation_guide?confirmationView(r.confirmation_guide,{compact:true}):setupLevels(r)}${setupTimes(r)}<button class="small" data-setup="${i}">${r.actionable?'Review trade':'View chart'}</button></article>`).join('')}</div>`;}


function bindSetups(rows,{drawer=false}={}){document.querySelectorAll('[data-setup]').forEach(button=>button.onclick=async()=>{
  const compact=rows[Number(button.dataset.setup)],id=compact?.setup?.setup_id;if(!id)return;
  button.disabled=true;
  try{
    let full;
    try{full=await read(`opportunity-detail-${id}`,`opportunities/${encodeURIComponent(id)}`);}
    catch(error){full={...compact,research_observations:{availability:'UNAVAILABLE',rows:[],missing_reason:`Detailed setup evidence is unavailable: ${plain(error.message)}`}};}
    if(!full)full={...compact,research_observations:{availability:'UNAVAILABLE',rows:[],missing_reason:'Detailed setup evidence is unavailable. The compact setup record is shown without research observations.'}};
    selectedSetup=full;
    if(!drawer){goto('trade');return;}
    const node=$('#opportunity-detail');if(!node)return;
    node.hidden=false;node.innerHTML=`<div class="card-head"><div><div class="eyebrow">SELECTED SETUP</div><h2>${esc(full.setup.symbol)} · ${esc(label(full.setup.direction))}</h2></div><button class="small" id="close-opportunity-detail">Close</button></div><h3>Why it exists</h3><p>${esc(setupSentence(full.primary_explanation))}</p>${signalMap?signalMap.disclosure(full.research_observations):''}<h3>Main reason to wait</h3><p>${esc(setupSentence(full.strongest_counterargument))}</p><button class="primary" id="open-selected-trade">Open in Trade</button>`;
    $('#close-opportunity-detail').onclick=()=>{node.hidden=true;button.focus();};
    $('#open-selected-trade').onclick=()=>goto('trade');node.focus({preventScroll:true});
  }catch(error){notify(plain(error.message));}finally{if(button.isConnected)button.disabled=false;}
});}
...
function recordedStopPanel(trade){
  const moves=trade.stop_history||[];
  const history=moves.length?`<ol>${moves.map(move=>`<li>${esc(date(move.effective_at))}: stop tightened to ${esc(move.stop)} to cover estimated costs after +1R.</li>`).join('')}</ol>`:'<p class="ticket-note">No stop adjustment was recorded for this trade.</p>';
  return `<section class="card section-gap"><h3>Actual protective stop</h3>${facts([['Original stop',trade.planned_stop],['Last recorded stop',trade.stop],['Rule for this trade',trade.profit_protection==='COST_COVER'?'Cost cover after +1R':'Original stop only']])}${history}</section>`;
}
...
async function drawTradeHistory(dialog,trade,comparison,recorded=false){
  ...
  try{
    ...
    if((trade.stop_history||[]).length&&setupPrice(trade.planned_stop)){
      const startBar=candles.find(c=>c.time<=start&&start<c.time+seconds);
      const path=startBar?[{time:startBar.time,value:Number(trade.planned_stop)}]:[];
      for(const move of trade.stop_history){
        const at=candles.find(c=>c.time===Number(move.effective_at));
        if(at&&setupPrice(move.stop))path.push({time:at.time,value:Number(move.stop)});
      }
      const last=candles.findLast(c=>c.time<=end);
      if(last&&path.length&&last.time>path[path.length-1].time)
        path.push({time:last.time,value:path[path.length-1].value});
      if(path.length>1){
        const actual=historyChart.addLineSeries({priceFormat,color:'#e4c36c',lineWidth:2,lineType:1,lastValueVisible:false,priceLineVisible:false,title:'Recorded stop'});
        actual.setData(path);
      }
    }
...
function bindDiagnoses(){document.querySelectorAll('[data-diagnosis]').forEach(button=>button.onclick=async()=>{
  try{
    ...
    dialog.querySelector('.trade-result-hero').insertAdjacentHTML('afterend',recordedStopPanel(trade)+excursionPanel(trade.excursion)+zoneStudyPanel(data.zone_comparison,true)+stopStudyPanel(data.stop_comparison,true));
...
async function botPage(){
  ...
  $('#content').innerHTML=heading('What the bot is doing.','Scanner activity, entry settings and recorded setup decisions.')+'<section class="card bot-panel" id="bot-status"><p class="loading">Reading the scanner’s latest report…</p></section><section class="card section-gap" id="bot-oi"><h2>Open-interest feed</h2><p>Reading collection health…</p></section><section class="card" id="bot-positions"><h2>Orders and trades managed by the bot</h2><p>Reading current orders…</p></section>';await Promise.all([loadBot(),loadOpenInterestStatus('#bot-oi')]);
}
...
async function opportunitiesPage(){if(workspace==='STOCKS'){$('#content').innerHTML=heading('Opportunities','Confirmed trade plans and developing setups.')+stockGate();return;}
  ...
  $('#content').innerHTML=heading('Opportunities','Ready to trade shows confirmed plans. Watching shows setups still developing. Account risk is checked again before an order.')+`<div class="segmented opportunity-tabs" role="tablist" aria-label="Setup view"><button type="button" id="ready-tab" role="tab" aria-selected="true" aria-controls="opportunity-panel" data-group="ready">Ready to trade <span data-count="ready"></span></button><button type="button" id="watching-tab" role="tab" aria-selected="false" tabindex="-1" aria-controls="opportunity-panel" data-group="watching">Watching <span data-count="watching"></span></button><button type="button" id="compare-tab" role="tab" aria-selected="false" tabindex="-1" aria-controls="opportunity-panel" data-group="compare">Compare</button></div><form id="filters" class="filters opportunity-filters"><button type="submit" class="small">Refresh setups</button><label>Market<input name="search" type="search" placeholder="Search BTC, ETH…" autocomplete="off"></label><label id="server-sort-label">Sort by<select name="sort" aria-label="Sort by"><option value="progress">Most developed first</option><option value="newest">Newest first</option><option value="distance">Nearest recorded zone</option></select></label><label id="compare-sort-label" hidden>Compare by<select name="compare_sort" aria-label="Compare by"><option value="newest">Newest confirmed first</option><option value="net_rr">Best reward after costs</option><option value="pool_against">Most room before a pool against you</option></select></label></form><section id="opportunity-panel" class="card" role="tabpanel" aria-labelledby="ready-tab"><p id="opportunity-summary" class="source-detail" role="status"></p><div id="opportunity-list" class="loading">Reading opportunities…</div><button id="show-more-setups" class="section-gap" hidden>Show 10 more</button></section><section id="opportunity-detail" class="card section-gap selected-setup-drawer" tabindex="-1" hidden></section><p class="source-detail">Setups that cannot proceed are kept in <a href="#blocked">Bot · blocked setups</a>.</p>`;
  ...
  const load=async()=>{if(!panel.isConnected)return;try{
    ...
    if(group==='compare'){
      // Open plans side by side. Search and the sort run on server values;
      // nothing here scores or ranks a trade.
      const form=new FormData(filters),term=String(form.get('search')||'').trim().toUpperCase();
      const data=await read('compare','opportunities/compare');if(!data)return;
      const items=data.items.filter(row=>!term||String(row.setup.symbol).toUpperCase().includes(term)).sort(COMPARE_SORTS[form.get('compare_sort')]||COMPARE_SORTS.newest);
      $('#opportunity-list').className='';
      $('#opportunity-list').innerHTML=compareTable(items);
      $('#opportunity-summary').textContent=`${items.length} open plan${items.length===1?'':'s'} · ${data.ordering}`;
      bindSetups(items,{drawer:true});
      return;
    }
    ...
    bindSetups(items,{drawer:true});updateConfirmationClocks();
  }catch(e){if(panel.isConnected)errorAt($('#opportunity-list'),e);}finally{if(panel.isConnected)panel.setAttribute('aria-busy','false');}};
  const choose=button=>{clearTimeout(timer);group=button.dataset.group;limit=10;$('#compare-sort-label').hidden=group!=='compare';$('#server-sort-label').hidden=group==='compare';for(const tab of document.querySelectorAll('[data-group]')){const active=tab===button;tab.setAttribute('aria-selected',String(active));tab.tabIndex=active?0:-1;}$('#opportunity-panel').setAttribute('aria-labelledby',button.id);void load();};
  ...
  // Every select, not the first: `$` is querySelector, and the Compare tab's
  // own sort control would otherwise change nothing.
  document.querySelectorAll('#filters select').forEach(select=>{select.onchange=()=>{clearTimeout(timer);limit=10;void load();};});
...
function drawResearchLayers(mainChart,priceSeries,data,enabled){
  const extraSeries=[],subcharts=[],chartHost=$('#chart');
  const svg=document.createElementNS('http://www.w3.org/2000/svg','svg');
  svg.classList.add('research-price-overlay');svg.setAttribute('aria-hidden','true');chartHost.append(svg);
  const summary=document.createElement('p');summary.className='research-layer-summary source-detail';
  const sequences=data.structure_sequences||[],completeSequenceCount=sequences.filter(row=>row.sweep_ts!=null).length;
  const regularCount=(data.regular_divergence||[]).length,hiddenCount=(data.hidden_divergence||[]).length;
  summary.textContent=`Research chart layers: ${(data.order_blocks||[]).length} order blocks; ${sequences.length} structure sequences (${completeSequenceCount} complete · ${sequences.length-completeSequenceCount} partial; partial means no sweep recorded); ${regularCount} regular divergence lines (solid teal); ${hiddenCount} hidden divergence lines (dashed violet). Missing open-interest intervals remain blank.`;
  chartHost.after(summary);
  const svgNode=(name,attributes,title)=>{const node=document.createElementNS('http://www.w3.org/2000/svg',name);for(const [key,value] of Object.entries(attributes))node.setAttribute(key,value);if(title){const caption=document.createElementNS('http://www.w3.org/2000/svg','title');caption.textContent=title;node.append(caption);}return node;};
  const drawOverlay=()=>{
    svg.replaceChildren();svg.setAttribute('viewBox',`0 0 ${chartHost.clientWidth} ${chartHost.clientHeight}`);
    const x=time=>mainChart.timeScale().timeToCoordinate(Number(time)),y=price=>priceSeries.priceToCoordinate(Number(price));
    if(enabled.has('orderblocks'))for(const block of data.order_blocks||[]){
      const x1=x(block.source_candle_ts),x2=x(block.break_ts),top=y(block.top),bottom=y(block.bottom);
      if([x1,x2,top,bottom].some(value=>value==null))continue;
      const left=Math.min(x1,x2),right=Math.max(x1,x2),upper=Math.min(top,bottom),lower=Math.max(top,bottom);
      if(right<0||left>chartHost.clientWidth||lower<0||upper>chartHost.clientHeight)continue;
      const bull=block.direction==='BULL',midX=(left+right)/2,midY=(upper+lower)/2;
      svg.append(svgNode('rect',{class:`research-ob ${bull?'bull':'bear'}`,x:left,y:upper,width:Math.max(3,right-left),height:Math.max(2,lower-upper)},`Order block · ${block.direction} · ${block.bottom} to ${block.top} · ${block.version}`));
      const blockLabel=svgNode('text',{class:`research-ob-label ${bull?'bull':'bear'}`,x:midX,y:midY,'text-anchor':'middle','dominant-baseline':'central'},`Order block · ${block.direction}`);
      blockLabel.textContent=`OB ${bull?'↑':'↓'}`;svg.append(blockLabel);
    }
    if(enabled.has('sequences'))for(const sequence of sequences){
      const block=(data.order_blocks||[]).find(item=>item.block_id===sequence.block_id);
      if(!block)continue;
      const breakX=x(sequence.break_ts),blockX=x(sequence.block_ts),midY=y((Number(block.top)+Number(block.bottom))/2),complete=sequence.sweep_ts!=null;
      const sweepX=complete?x(sequence.sweep_ts):null;
      if([breakX,blockX,midY].some(value=>value==null)||(complete&&sweepX==null))continue;
      const points=complete?`${sweepX},${midY} ${breakX},${midY-12} ${blockX},${midY}`:`${blockX},${midY} ${breakX},${midY-12}`;
      const title=complete?`Sweep → break → linked block · ${sequence.version}`:`Partial sequence · break linked to block · sweep unavailable · ${sequence.version}`;
      svg.append(svgNode('polyline',{class:`research-sequence ${complete?'complete':'partial'}`,'data-sequence-state':complete?'COMPLETE':'PARTIAL',points},title));
      const labels=complete?[['S',sweepX,midY-4],['B',breakX,midY-16],['OB',blockX,midY-4]]:[];
      for(const [label,labelX,labelY] of labels){const text=svgNode('text',{class:'research-layer-label',x:labelX,y:labelY});text.textContent=label;svg.append(text);}
      if(!complete){const state=svgNode('text',{class:'research-layer-label research-sequence-state',x:(breakX+blockX)/2,y:midY-6,'text-anchor':'middle'});state.textContent='B→OB · partial';svg.append(state);}
    }
  };
  const markers=[];
  const blockIds=new Set((data.order_blocks||[]).map(row=>row.block_id));
  if(enabled.has('sequences'))for(const row of sequences){
    if(!blockIds.has(row.block_id))continue;
    const breakX=mainChart.timeScale().timeToCoordinate(Number(row.break_ts)),blockX=mainChart.timeScale().timeToCoordinate(Number(row.block_ts));
    if([breakX,blockX].some(value=>value==null)||Math.max(breakX,blockX)<0||Math.min(breakX,blockX)>chartHost.clientWidth)continue;
    if(row.sweep_ts&&mainChart.timeScale().timeToCoordinate(Number(row.sweep_ts))!=null)markers.push({time:Number(row.sweep_ts),position:row.direction==='BULL'?'belowBar':'aboveBar',shape:'circle',color:'#f0c37e',text:'Sweep'});
    markers.push({time:Number(row.break_ts),position:row.direction==='BULL'?'belowBar':'aboveBar',shape:row.direction==='BULL'?'arrowUp':'arrowDown',color:'#f0c37e',...(row.sweep_ts?{text:'Break → OB'}:{})});
  }
  priceSeries.setMarkers(markers.sort((a,b)=>a.time-b.time));
  summary.dataset.sequenceMarkers=String(markers.length);
  let regularSeries=0,hiddenSeries=0;
  if(enabled.has('divergence'))for(const row of [...(data.regular_divergence||[]).map(v=>({...v,hidden:false})),...(data.hidden_divergence||[]).map(v=>({...v,hidden:true}))]){
    if(!setupPrice(row.price_prev)||!setupPrice(row.price))continue;
    const line=mainChart.addLineSeries({color:row.hidden?'#b8a4ff':'#8de5e9',lineWidth:2,lineStyle:row.hidden?2:0,priceLineVisible:false,lastValueVisible:false});
    line.setData([{time:Number(row.prev_pivot_ts),value:Number(row.price_prev)},{time:Number(row.time),value:Number(row.price)}]);extraSeries.push(line);if(row.hidden)hiddenSeries++;else regularSeries++;
  }
  summary.dataset.regularDivergenceSeries=String(regularSeries);summary.dataset.hiddenDivergenceSeries=String(hiddenSeries);
  const parent=$('#research-indicator-panes');parent.hidden=!(enabled.has('stochrsi')||enabled.has('openinterest'));
  const pane=(id,title,last,points,gaps=0)=>{const section=document.createElement('section');section.className='research-indicator-pane';section.dataset.researchPane=id;section.dataset.pointCount=String(points);section.dataset.gapCount=String(gaps);section.setAttribute('role','group');section.setAttribute('aria-label',title);section.innerHTML=`<header><b>${esc(title)}</b><small>${esc(last)}</small></header><div class="indicator-canvas" aria-hidden="true"></div><div class="indicator-resizer" role="separator" tabindex="0" aria-label="Resize ${esc(title)} pane" aria-orientation="horizontal" aria-valuemin="90" aria-valuemax="250" aria-valuenow="133" aria-valuetext="133 pixels high"></div>`;parent.append(section);const canvas=section.querySelector('.indicator-canvas'),c=LightweightCharts.createChart(canvas,{width:parent.clientWidth,height:105,layout:{background:{color:'#111a23'},textColor:'#899eaf'},grid:{vertLines:{color:'#202d38'},horzLines:{color:'#202d38'}},rightPriceScale:{borderColor:'#334352'},timeScale:{borderColor:'#334352',timeVisible:true},handleScroll:false,handleScale:false});subcharts.push(c);const handle=section.querySelector('.indicator-resizer'),size=next=>{next=Math.max(90,Math.min(250,next));section.style.height=`${next}px`;handle.setAttribute('aria-valuenow',String(Math.round(next)));handle.setAttribute('aria-valuetext',`${Math.round(next)} pixels high`);c.applyOptions({width:parent.clientWidth,height:Math.max(55,next-28)});};handle.onkeydown=e=>{if(!['ArrowUp','ArrowDown'].includes(e.key))return;e.preventDefault();size(section.clientHeight+(e.key==='ArrowUp'?12:-12));};handle.onpointerdown=e=>{e.preventDefault();const startY=e.clientY,startHeight=section.clientHeight;handle.setPointerCapture(e.pointerId);handle.onpointermove=move=>size(startHeight+move.clientY-startY);handle.onpointerup=()=>{handle.onpointermove=null;};};return c;};
  if(enabled.has('stochrsi')){const rows=(data.stoch_rsi||[]).filter(r=>r.k!=null&&r.d!=null),last=rows.at(-1);const c=pane('stoch','Stoch RSI · K / D · 20 / 80',last?`Last closed ${date(last.confirmed_at)}`:'Unavailable',rows.length),k=c.addLineSeries({color:'#78e2b6',lineWidth:2,priceLineVisible:false}),d=c.addLineSeries({color:'#f0c37e',lineWidth:1,lineStyle:2,priceLineVisible:false});k.setData(rows.map(r=>({time:Number(r.time),value:Number(r.k)})));d.setData(rows.map(r=>({time:Number(r.time),value:Number(r.d)})));for(const level of [20,80])k.createPriceLine({price:level,color:'#526273',lineWidth:1,lineStyle:2,axisLabelVisible:true,title:String(level)});}
  if(enabled.has('openinterest')){const rows=data.open_interest||[],last=[...rows].reverse().find(r=>r.value!=null),gaps=rows.filter(r=>r.value==null).length;const c=pane('oi','Open interest · value and one-hour change',last?`Observed ${date(last.time)} · Value ${last.value} · Δ1H ${last.change_1h??'unavailable'}`:'Unavailable',rows.length-gaps,gaps),line=c.addLineSeries({color:'#8dbbff',lineWidth:2,priceLineVisible:false});line.setData(rows.map(r=>r.value==null?{time:Number(r.time)}:{time:Number(r.time),value:Number(r.value)}));}
  // The price chart is the sole time-window controller. A newly-created OI
  // pane can have only a handful of observations, so allowing a lower pane to
  // drive the main chart would turn those points into giant candles.
  let syncing=false,disposed=false,readyTimer;
  const alignPanesToPrice=()=>{
    const range=mainChart.timeScale().getVisibleRange();if(!range)return;
    syncing=true;for(const target of subcharts)try{target.timeScale().setVisibleRange(range);}catch(error){void error;}syncing=false;
  };
  alignPanesToPrice();
  const rangeListener=range=>{drawOverlay();if(syncing||!range)return;alignPanesToPrice();};
  mainChart.timeScale().subscribeVisibleTimeRangeChange(rangeListener);
  readyTimer=setTimeout(()=>{if(!disposed)alignPanesToPrice();},0);
  const overlayResize=new ResizeObserver(drawOverlay);overlayResize.observe(chartHost);drawOverlay();
  return ()=>{disposed=true;clearTimeout(readyTimer);mainChart.timeScale().unsubscribeVisibleTimeRangeChange(rangeListener);overlayResize.disconnect();svg.remove();summary.remove();priceSeries.setMarkers([]);for(const item of extraSeries)mainChart.removeSeries(item);for(const item of subcharts)item.remove();parent.replaceChildren();parent.hidden=true;};
}

async function drawChart(symbol,tf){disposeChart();try{const data=await read('candles','candles',{symbol,tf});if(!data||!$('#chart'))return;
    let guide;
    if(selectedSetup&&selectedSetup.setup.symbol===symbol&&selectedSetup.setup.timeframe===tf){
      const guideSetupId=selectedSetup.setup.setup_id;
      try{guide=await read('setup-guide','setup-guide',{setup_id:guideSetupId});}catch(error){notify(`Chart levels unavailable: ${plain(error.message)}`);}
      if(!$('#chart')||selectedSetup?.setup.setup_id!==guideSetupId)return;
      const node=$('#confirmation-detail');
      if(node&&guide){node.innerHTML=confirmationView(guide);updateConfirmationClocks();}
      const evidenceNode=$('#trade-evidence');
      if(evidenceNode)evidenceNode.innerHTML=guide?.trade_evidence?evidenceView(guide.trade_evidence):'<p class="ticket-note">Trade evidence is unavailable for this setup.</p>';
      const signalNode=$('#setup-signal-map');
      if(signalNode&&signalMap){
        let currentResearch=null;
        try{
          const insight=await selection.read(`signal-now-${guideSetupId}`,`/api/chart-insight?symbol=${encodeURIComponent(symbol)}&tf=${encodeURIComponent(tf)}&setup_id=${encodeURIComponent(guideSetupId)}`);
          currentResearch=insight?.research_observations||null;
        }catch(error){currentResearch={availability:'UNAVAILABLE',rows:[],missing_reason:'Current research observations are unavailable. The setup-time record is unchanged.'};}
        if(signalNode.isConnected&&selectedSetup?.setup.setup_id===guideSetupId)
          signalNode.innerHTML=signalMap.trade(selectedSetup.research_observations,currentResearch);
      }
    }
    $('#chart-title').textContent=`${symbol} / ${tf}`;$('#chart-caption').textContent=`Latest completed candle ${data.mark??'unavailable'} · ${date(data.observed_at)}`;if(!data.candles.length){$('#chart').className='chart-empty';$('#chart').textContent='No completed candles are available for this market yet. Try another market or wait for the next scan.';return;}$('#chart').className='chart';$('#chart').textContent='';
    ...
    if(guide){
      addLevel(guide.confirmation_boundary,'Confirmation edge','#a4baff');
    ...
    const researchKeys=['orderblocks','sequences','divergence','stochrsi','openinterest'];
    let enabled=new Set(),researchSeries=null;
    const renderResearch=async()=>{
      chartLayersCleanup?.();chartLayersCleanup=null;
      if(!enabled.size||!chart||!$('#research-indicator-panes'))return;
      const caption=$('#chart-caption');
      try{
        researchSeries??=await selection.read(
          `research-series-${symbol}-${tf}`,
          `/api/research-series?symbol=${encodeURIComponent(symbol)}&tf=${encodeURIComponent(tf)}`,
        );
        if(chart!==chartRequest||!$('#research-indicator-panes')||$('#symbol')?.value.trim()!==symbol||$('#tf')?.value!==tf)return;
        chartLayersCleanup=drawResearchLayers(chart,series,researchSeries,enabled);
      }catch(error){
        if(caption)caption.innerHTML+=` <span class="error">Research layers unavailable: ${esc(plain(error.message))}</span>`;
      }
    };
    const selectPreset=(preset,render=true)=>{
      const isResearch=preset==='research';
      enabled=new Set(isResearch?researchKeys:[]);
      document.querySelectorAll('[data-chart-preset]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.chartPreset===preset)));
      document.querySelectorAll('[data-research-layer]').forEach(input=>{input.checked=enabled.has(input.dataset.researchLayer);});
      localStorage.setItem('ss.cockpit-chart-preset.v1',preset);
      if(render)void renderResearch();
    };
    document.querySelectorAll('[data-chart-preset]').forEach(button=>button.onclick=()=>selectPreset(button.dataset.chartPreset));
    document.querySelectorAll('[data-research-layer]').forEach(input=>input.onchange=()=>{
      if(input.checked)enabled.add(input.dataset.researchLayer);else enabled.delete(input.dataset.researchLayer);
      document.querySelectorAll('[data-chart-preset]').forEach(button=>button.setAttribute('aria-pressed','false'));
      localStorage.setItem('ss.cockpit-chart-preset.v1','custom');
      void renderResearch();
    });
    const savedPreset=localStorage.getItem('ss.cockpit-chart-preset.v1');
    selectPreset(['clean','trade','research'].includes(savedPreset)?savedPreset:'trade',false);
    if(enabled.size)void renderResearch();
...
function trialPanel(t){
  ...
  return `<section class="card trial-panel"><div class="card-head"><div><div class="eyebrow">NEW SETUPS · FORWARD TRIAL</div><h2>Breakout & retest</h2></div>${badge(t.state==='PAUSED'?'Paused':'Trial enabled')}</div><p>Testing new setups as prices arrive. This has its own simulated money and does not place orders in your paper account.</p>${t.note?`<div class="banner">${esc(t.note)}</div>`:''}<div class="grid equal"><div><span class="source-detail">Trial balance · closed trades only</span><h3 class="trial-balance">${money(t.balance)}</h3><span class="source-detail">Started with ${money(t.starting_balance)}</span></div><div><span class="source-detail">Result after modeled costs</span>${pnl(t.pnl_usd)}</div></div>${points.length>1?`<svg class="trial-curve" viewBox="0 0 600 160" role="img" aria-label="Trial balance after each completed trade, from ${esc(money(points[0].value))} to ${esc(money(t.balance))}"><path d="M20 145H580" stroke="currentColor" opacity=".25"/><polyline points="${coords}" fill="none" stroke="currentColor" stroke-width="3"/></svg><p class="source-detail">Balance after each completed trade, in order. Open trades are not included.</p>`:'<div class="trial-empty">The balance chart will appear after the first completed trade.<br><span class="source-detail">No results yet — the trial is gathering evidence.</span></div>'}<div class="trial-counts">${Object.entries(states).map(([key,text])=>`<div><strong>${counts[key]||0}</strong><span>${text}</span></div>`).join('')}</div><p class="source-detail">Started ${esc(date(t.started_at))}<br>Last checked ${esc(date(t.checked_at))}. Updates after each scanner pass.</p>${restartNote(t)}<details><summary>Trial rules & costs</summary><p>Targets ${money(t.risk_usd)} of price risk per trade, with up to ${esc(t.max_slots)} orders or trades open and one per market. Trade sizes are fixed when the order is planned; a different fill can change the amount at risk. Trading costs can make a loss larger.</p><p>Uses its own cash limits, without borrowing. Fresh setups get two future candles to reach the limit price, then a market entry on the following candle. This entry window starts after detection; older or already-expired setups are excluded. Fees, slippage and funding are modeled. Missing price data holds up the result. These results do not establish that the strategy is profitable.</p></details><div class="trial-trades">${(t.items||[]).map(r=>`<details class="trial-trade"><summary><span><strong>${esc(r.symbol)}</strong> · ${esc(r.tf)}<small>${esc(date(r.observed_at))}</small></span><span>${esc(states[r.state]||r.state)}${r.pnl_usd!=null?`<strong class="pnl-${pnlTone(r.pnl_usd)}">${money(r.pnl_usd)}</strong>`:''}</span></summary><p>${esc(r.reason||label(r.result?.outcome)||'Waiting for the next update.')}</p>${facts([['Direction',label(r.direction)],[r.fill?'Filled entry':'Planned entry',r.entry],['Stop-loss',r.sl],['Target',r.tp]])}${r.result?facts([['Exit price',r.result.exit],['Fees',money(r.result.fees_usd)],['Modeled funding',money(r.result.funding_usd)],['Completed',date(r.result.at)]]):''}${r.fill?.note?`<p>${esc(r.fill.note)}</p>`:''}${r.result?.ambiguous?'<p>Both stop and target were reached in one candle. The trial counted the stop-loss.</p>':''}${r.result?.slip_missing?'<p>Exit slippage could not be estimated. This result has incomplete costs.</p>':''}</details>`).join('')}</div></section>`;
}

function simpleStrategyPanel(t){
  if(!t)return `<section class="card section-gap"><h2>Simple strategy comparison</h2><p role="alert">Comparison status is unavailable.</p></section>`;
  if(t.state==='NOT_STARTED')return `<section class="card section-gap"><h2>Simple strategy comparison</h2><p>Collection has not started.</p></section>`;
  if(t.state==='UNAVAILABLE')return `<section class="card section-gap"><h2>Simple strategy comparison</h2><p>${esc(t.note||'Unavailable for this market.')}</p></section>`;
  const names={CURRENT:'Current pullback & reversal',CHANNEL:'Simple 4H channel breakout',TREND:'Trend pullback & reclaim'};
  const arms=t.arms||{};
  const rows=Object.entries(names).map(([key,name])=>{const a=arms[key]||{},long=a.by_direction?.LONG||{},short=a.by_direction?.SHORT||{};return `<article class="study-arm"><h3>${esc(name)}</h3>${facts([['Completed trades',a.closed??0],['Markets represented',a.symbols??0],['Net result after modeled costs',a.net_usd==null?'Waiting for results':money(a.net_usd)],['Return',a.return_pct==null?'Waiting for results':`${esc(a.return_pct)}%`],['Worst drawdown',a.max_drawdown_pct==null?'Waiting for results':`${esc(a.max_drawdown_pct)}%`],['Longs: trades / net',`${long.closed??0} / ${money(long.net_usd??'0')}`],['Shorts: trades / net',`${short.closed??0} / ${money(short.net_usd??'0')}`],['Average R',a.mean_r??'Waiting for results'],['Open orders or trades',a.unresolved??0]])}</article>`;}).join('');
  const p=t.primary||{},i=p.interval,verdict={UNKNOWN:'Too few completed trades',PROMISING_UNPROVEN:'Promising, still unproven',NO_RELIABLE_IMPROVEMENT:'No reliable improvement'}[p.verdict]||'Collecting evidence';
  return `<section class="card section-gap"><div class="card-head"><div><div class="eyebrow">FUTURE DATA · RESEARCH ONLY</div><h2>Does a simpler strategy work better?</h2></div>${badge(t.state==='PAUSED'?'Paused':'Collecting')}</div><p>Three independent simulated books follow fresh 4-hour signals. None places an order in your account or changes which setups qualify.</p>${t.reason?`<p role="alert">${esc(t.reason)}</p>`:''}<p><strong>${esc(verdict)}</strong></p><p class="source-detail">The main test compares the channel breakout with current pullback and reversal setups. Trend pullback and reclaim is exploratory.</p><div class="study-arms">${rows}</div>${facts([['Minimum per main group',`${p.minimum_trades_each??30} completed trades and ${p.minimum_symbols_each??8} markets`],['Channel minus current, average net R',i?`${Number(i.uplift_r).toFixed(3)}R`:'Not gradeable'],['95% cluster interval',i?`[${Number(i.ci_lo).toFixed(3)}, ${Number(i.ci_hi).toFixed(3)}]R`:'Not gradeable'],['Collection started',t.started_at?date(t.started_at):'Not started'],['Collection ends',t.ends_at?date(t.ends_at):'Not scheduled'],['Last checked',t.checked_at?date(t.checked_at):'Not checked']])}<p class="source-detail">This is a prospective paper comparison with fees, slippage and funding modeled. A small sample cannot prove an edge. Used in trading: No.</p></section>`;
}

function signalResearchPanel(report){
  if(!report)return `<section class="card section-gap" id="research-signals"><h2>Which signals have earned trust?</h2><p>Data unavailable. No missing reading is shown as zero or neutral.</p></section>`;
  const cards=(report.rows||[]).map(row=>{
    const p=row.progress||{},interval=row.ci_lo==null?'Not gradeable':`[${Number(row.ci_lo).toFixed(3)}, ${Number(row.ci_hi).toFixed(3)}]R`;
    const counts=`Signal present: ${row.exposed_count} trades · absent: ${row.control_count} trades`;
    return `<details class="signal-evidence-card"><summary><span><strong>${esc(row.label)}</strong><small>${esc(counts)}</small></span><span class="research-status">${esc(row.verdict)}</span></summary><div class="signal-evidence-detail"><p>${esc(row.hypothesis)}</p>${facts([['Signal present',`${row.exposed_count} trades · ${row.exposed_symbol_clusters} symbols`],['Signal absent',`${row.control_count} trades · ${row.control_symbol_clusters} symbols`],['Progress',`Present ${p.exposed_trades}/${p.trades_required_each} trades, ${p.exposed_symbols}/${p.symbols_required_each} symbols · Absent ${p.control_trades}/${p.trades_required_each} trades, ${p.control_symbols}/${p.symbols_required_each} symbols`],['Coverage / missing',`${Math.round(Number(row.coverage||0)*100)}% / ${Math.round(Number(row.missing_rate||0)*100)}%`],['Net-R uplift after costs',row.uplift_r==null?'Not gradeable':`${Number(row.uplift_r).toFixed(3)}R`],['95% interval',interval],['Stability',row.sample_ok?(row.stable?'Stable across the time split':'Not stable'):'Not gradeable'],['Corrected significance',row.q_value==null?'Not gradeable':Number(row.q_value).toFixed(3)],['Collection start',row.collection_start?date(row.collection_start):'Not started'],['Detector version',row.detector_version]])}<details><summary>Other patterns observed</summary><ul>${(row.other_patterns_observed||[]).map(item=>`<li>${esc(item.label)} <small>${item.status==='EXPLORATORY_UNCOLLECTED'?'Planned exploratory pattern — no observations collected.':'Exploratory — cannot be Proven useful'}</small></li>`).join('')||'<li>None reported</li>'}</ul></details></div></details>`;
  }).join('');
  return `<section class="card signal-research section-gap" id="research-signals"><div class="eyebrow">SIGNALS · RESEARCH ONLY</div><h2>Which signals have earned trust?</h2><p class="signal-research-verdict">${esc(report.verdict||'Data unavailable')}</p><p>Each signal compares completed trades where it was present at setup time with trades where it was absent. Neither group can be judged until it has ${esc(report.minimums?.closed_trades_each||30)} trades across ${esc(report.minimums?.symbol_clusters_each||8)} markets.</p><p class="ticket-note">Used in trading: No. Open a signal to see its test, data coverage and statistics.</p><div class="signal-evidence-grid">${cards||'<p>No signal tests reported yet.</p>'}</div></section>`;
}

function researchStudyDisclosure(title,status,content){
  return `<details class="card research-study section-gap"><summary><span><strong>${esc(title)}</strong><small>Open comparison and method</small></span><span class="research-status">${esc(status)}</span></summary><div class="research-study-body">${content}</div></details>`;
}

function researchPairStatus(study){
  if(!study||study.state==='UNAVAILABLE')return 'Data unavailable';
  if(study.state==='NOT_STARTED')return 'Not started';
  if(study.state==='PAUSED')return 'Paused';
  return `${study.paired_count||0} complete comparisons`;
}

function openInterestPanel(payload){const oi=payload?.open_interest;if(!oi)return `<h2>Open-interest feed</h2><p class="error">Collection status unavailable. Missing values are not shown as zero.</p><strong>Collecting only — unused by trading.</strong>`;return `<h2>Open-interest feed</h2>${facts([['Supported contracts',oi.supported_contract_count??'Not reported'],['Last successful observation',oi.last_successful_observation?date(oi.last_successful_observation):'Not reported'],['Freshness',label(oi.freshness)],['Missing contract observations',oi.missing_contract_observations??'Not reported'],['Failed collection attempts',oi.failed_collection_attempts??'Not reported'],['Collection start',oi.collection_start?date(oi.collection_start):'Not started']])}<strong>Collecting only — unused by trading.</strong>${oi.last_error?`<p class="error section-gap">Latest feed failure: ${esc(oi.last_error)}</p>`:''}`;}
async function loadOpenInterestStatus(selector){const node=$(selector);if(!node)return;try{const data=await read('phemex-status','phemex/status');if(data&&node.isConnected)node.innerHTML=openInterestPanel(data);}catch(error){if(node.isConnected)node.innerHTML=openInterestPanel(null);}}
...
async function researchPage(){const data=await read('research','research');if(!data)return;if(workspace==='STOCKS'){const t=data.training;$('#content').innerHTML=heading('Learn the stock workflow.','Practice with made-up prices. These examples do not affect your account.')+`<div class="banner">${esc(plain(t.disclaimer))}</div><section class="card"><h2>Training scenarios</h2><div class="table-wrap"><table><thead><tr><th>Scenario</th><th>Readiness</th><th>Evidence</th></tr></thead><tbody>${t.setups.map(s=>`<tr><td>${esc(s.name)}</td><td>${badge(s.state)}</td><td>${esc(plain((s.rejections||[]).map(x=>x.detail).join(' ')||'This practice setup meets the example rules.'))}</td></tr>`).join('')}</tbody></table></div></section><section class="card section-gap"><h2>Practice trade result</h2>${facts([['Outcome',label(t.simulation?.outcome)],['Result',t.simulation?.r_multiple+' R'],['Data','Practice data · trading fees not included'],['Counts toward bot performance','Never']])}</section>`;return;}
  ...
  const [trial,study,zone,simple]=await Promise.all([read('forward-trial','forward-trial'),read('stop-comparison','stop-comparison'),read('zone-comparison','zone-comparison'),read('simple-strategy-trial','simple-strategy-trial')].map(request=>request.catch(()=>null)));
  if(generation!==selection.generation)return;
  const simpleStatus=!simple||simple.state==='UNAVAILABLE'?'Data unavailable':simple.state==='NOT_STARTED'?'Not started':simple.state==='PAUSED'?'Paused':simple.primary?.verdict==='UNKNOWN'?'Too few completed trades':simple.primary?.verdict?label(simple.primary.verdict):'Collecting evidence';
  const trialStatus=!trial?'Data unavailable':trial.state==='PAUSED'?'Paused':trial.state==='NOT_STARTED'?'Not started':`${trial.counts?.CLOSED||0} completed trades`;
  const records=data.items.slice(0,50);
  $('#content').innerHTML=heading('Research is evidence.','See what has been learned, what is still collecting, and the records behind each study.')+`<nav class="research-jump" aria-label="Research sections"><button type="button" data-research-jump="research-signals">Signals</button><button type="button" data-research-jump="research-strategies">Strategy tests</button><button type="button" data-research-jump="research-records">Setup records</button></nav>`+signalResearchPanel(data.signals)+`<section id="research-strategies" class="research-group section-gap"><h2>Strategy tests</h2><p>These compare simulated rules on future data. Open a study to see the numbers and how it works. None changes your account.</p></section>`+researchStudyDisclosure('Simpler strategy',simpleStatus,simpleStrategyPanel(simple))+researchStudyDisclosure('Defended zones',researchPairStatus(zone),zoneStudyPanel(zone)||'<p>Comparison data unavailable.</p>')+researchStudyDisclosure('Protective stops',researchPairStatus(study),stopStudyPanel(study)||'<p>Comparison data unavailable.</p>')+researchStudyDisclosure('Breakout and retest',trialStatus,trialPanel(trial))+`<section class="card section-gap" id="research-records"><h2>Simulation results & setup decisions</h2><p>${records.length} recent records. These are simulated outcomes and setup decisions, not account trades.</p><details class="research-records"><summary>Browse recent setup records</summary>${researchCards(records)}</details></section><div class="banner info section-gap">${esc(plain(data.note))} Results marked R scale by the amount risked on a trade: 2R is twice that amount.</div><section class="card section-gap"><h2>How the bot trades</h2><p>Learn about pullbacks, reversals, and the checks a setup must pass before the bot can place an order.</p><a class="text-link" href="#strategies">Explore the strategies →</a></section><section id="research-detail" class="card section-gap" hidden></section>`;document.querySelectorAll('[data-research-jump]').forEach(button=>{button.onclick=()=>document.getElementById(button.dataset.researchJump)?.scrollIntoView({block:'start'});});document.querySelectorAll('[data-setup]').forEach(button=>{button.textContent='Inspect';button.onclick=()=>{const r=records[Number(button.dataset.setup)];const node=$('#research-detail');node.hidden=false;node.innerHTML=setupExplanation(r);node.scrollIntoView({block:'nearest'});};});}

async function settingsPage(){const [creds,oiStatus,tradeSettings]=await Promise.all([selection.read('credentials','/api/credentials'),workspace==='CRYPTO'?read('phemex-status','phemex/status').catch(()=>null):Promise.resolve(null),workspace==='CRYPTO'?selection.read('trade-settings','/api/settings').catch(()=>null):Promise.resolve(null)]);if(!creds)return;const stock=workspace==='STOCKS';const targets=Object.keys(creds.target_fields).filter(t=>stock?['alpaca-paper','massive-stocks'].includes(t):!['alpaca-paper','massive-stocks'].includes(t));

  $('#content').innerHTML=heading('Manage your account.','Choose a market, connect supported providers, and start in paper.')+`<div class="grid equal"><section class="card"><h2>1 / Market & account</h2><p>${stock?'Stock trading and scanning are not available yet. Practice examples are available in Research.':'Paper trading uses public prices. You do not need exchange keys to place simulated orders.'}</p>${stock?stockGate():facts([['Current account','Paper trading'],['Risk per trade',context.risk_percent_label],['Maximum positions',context.slot_ceiling],['Account status',label(context.state)]])}<hr><h2>2 / Provider connections</h2><p class="ticket-note">Saved connection details still need to be checked. Your keys are stored securely on this computer.</p>${targets.map(target=>`<details class="provider"><summary>${esc(target)} · ${Object.values(creds.status[target]||{}).some(Boolean)?'Details saved':'Not connected'}</summary><form data-provider="${esc(target)}">${creds.target_fields[target].map(field=>`<div class="field"><label for="${target}-${field}">${esc(field.replaceAll('_',' '))} ${creds.status[target]?.[field]?'· stored':''}</label><input type="password" id="${target}-${field}" name="${field}" autocomplete="new-password" placeholder="Leave blank to keep existing"></div>`).join('')}<button type="submit" ${!creds.available?'disabled':''}>Save connection details</button></form>${stock?`<button data-verify="${target}">Check data access</button>`:'<p class="source-detail">Paper trading uses public prices and does not need exchange keys. Real-money trading is not enabled.</p>'}${target==='phemex-perp'?`<section class="oi-feed section-gap">${openInterestPanel(oiStatus)}</section>`:''}<div id="verify-${target}" class="source-detail section-gap"></div></details>`).join('')}</section><div class="stack"><section class="card"><h2>3 / Risk & paper</h2>${stock?'<p>Stock paper trading is not available yet.</p>':`<form id="risk-settings"><label for="account-risk">Risk per trade (%)</label><input id="account-risk" name="risk_percent" type="number" min="0.000001" max="100" step="any" required value="${esc(context.risk_percent_label.replace('%',''))}"><p class="ticket-note">The share of your account balance you can risk on each new trade, whether placed by you or the bot. Existing orders keep their size. New orders pause if daily losses reach four times this percentage. You can hold one trade at a time.</p><button type="submit">Save paper risk</button></form>${facts([['Bot mode',label(context.automation.mode)],['Entry pause',context.automation.halted?'Paused':'Not paused']])}<div class="actions section-gap"><button id="mode-paper" ${context.automation.mode==='PAPER'?'disabled':''}>Enable paper bot</button><button id="mode-off" ${context.automation.mode==='OFF'?'disabled':''}>Turn bot off</button></div><p class="ticket-note">Turning the bot off stops it from placing new orders. Existing stop-losses and targets stay active. Orders you place still follow your account limits.</p><hr><h3>Start a new paper account</h3><p class="ticket-note">Pause new orders and wait for current trades and orders to finish. Then start a new $10,000 paper account with your saved risk setting. Your history is kept, and trades are never closed automatically to make this happen.</p><div class="actions"><button data-cutover="drain" ${context.state==='DRAINING'?'disabled':''}>Pause orders to prepare</button><button data-cutover="resume" ${context.state!=='DRAINING'?'disabled':''}>Keep using this account</button><button data-cutover="complete" class="primary" ${context.state!=='DRAINING'||context.cutover_blockers.length?'disabled':''}>Start new paper account</button></div>${context.cutover_blockers.length?`<p class="source-detail section-gap">${context.cutover_blockers.length} order(s) or position(s) still need to finish. <a href="#trade">View open trades and orders →</a></p>`:''}`}</section><section class="card"><div class="card-head"><h2>Live execution</h2>${badge('LOCKED','warning')}</div><p>Real-money trading is not available. Exchange safety checks still need to be completed.</p><a class="text-link" href="#strategies">Read how the bot trades →</a></section><section class="card"><h2>Display & recovery</h2><p>Learn how the bot identifies setups and decides whether an order is allowed.</p><a href="#strategies" class="text-link">Explore the strategies →</a><p class="ticket-note">Research explains possible trades. Your Journal shows what was actually placed.</p></section></div></div>`;

  if(!stock){
    const enabled=tradeSettings?.values?.profit_protection_cost_cover;
    $('#risk-settings').insertAdjacentHTML('afterend',`<form id="profit-stop-setting" class="section-gap"><h3>Profit protection for new bot trades</h3><label class="field"><input type="checkbox" id="profit-stop-toggle" ${enabled?'checked':''} ${enabled==null?'disabled':''}> Move the stop to cover estimated costs after a full setup candle survives and price reaches +1R</label><p class="ticket-note">Off by default. Paper and private custody use the same closed-candle rule. Existing trades keep the choice recorded when they were placed. A stop can only tighten. Switching this starts a new forward results window.</p>${enabled==null?'<p class="error" role="alert">Profit-protection setting is unavailable. No change can be saved.</p>':''}<button type="submit" ${enabled==null?'disabled':''}>Save profit protection</button></form>`);
    $('#profit-stop-setting').onsubmit=async event=>{event.preventDefault();const chosen=$('#profit-stop-toggle').checked;if(chosen===enabled){notify('Profit protection is already set this way.');return;}if(!await confirmAction('Change profit protection?',`New bot trades will ${chosen?'use':'not use'} the cost-cover stop rule. Open trades keep their recorded choice. This starts a new forward results window.`,chosen?'Turn on':'Turn off'))return;try{const result=await post('/api/settings',{changes:{profit_protection_cost_cover:chosen},note:'Cockpit profit-protection control'});if(result){notify('Profit-protection setting saved for new bot trades.');await render();}}catch(error){notify(error.message);}};
  }
...
setInterval(updateConfirmationClocks,1000);
