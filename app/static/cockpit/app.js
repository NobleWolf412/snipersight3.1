...
async function settingsPage(){const [creds,oiStatus]=await Promise.all([selection.read('credentials','/api/credentials'),workspace==='CRYPTO'?read('phemex-status','phemex/status').catch(()=>null):Promise.resolve(null)]);if(!creds)return;const stock=workspace==='STOCKS';const targets=Object.keys(creds.target_fields).filter(t=>stock?['alpaca-paper','massive-stocks'].includes(t):!['alpaca-paper','massive-stocks'].includes(t));
