...
/* ═══════════════ THE A–Z ═══════════════
   Every term the app underlines, in one searchable place on Learn.

   The tooltips are a reminder for a word you half-know. They are no use at
   all if you do not know WHICH word you are looking for, and nothing tied
   the 67 of them together — there was no way to browse the vocabulary, only
   to stumble into it. This is that page.

   It reads the same GLOSSARY object the tooltips do, so a term can never be
   defined twice or drift between the two. */
(() => {
  ...
  // camelCase keys are the engine's; the heading is the word a person says.
  ...
  function render(filter){
    ...
    const entries = Object.entries(window.GLOSSARY || {})
      ...
      .map(([k, v]) => [pretty(k), v, k])
      .filter(([name, def]) => !q ||
        name.toLowerCase().includes(q) || def.toLowerCase().includes(q))
      .sort((a, b) => a[0].localeCompare(b[0]));

    const count = document.getElementById('glCount');
    if(count) count.textContent = q
      ? `${entries.length} of ${Object.keys(window.GLOSSARY || {}).length}`
      : `${entries.length} terms`;

    if(!entries.length){
      // The empty-window rule: nothing MATCHED is not the same as nothing exists.
      root.innerHTML = `<div class="empty">No term matches “${esc(q)}”. ` +
        `The glossary covers the words this app puts on screen — if you met ` +
        `it somewhere else, it may not be here.</div>`;
      return;
    }

    let letter = '';
    root.innerHTML = entries.map(([name, def, key]) => {
      const first = name[0].toUpperCase();
      const head = first !== letter ? (letter = first,
        `<h3 class="gl-letter" id="gl-${first}">${first}</h3>`) : '';
      return head + `<div class="gl-entry" id="gl-term-${esc(key)}">
        <dt class="gl-term">${esc(name)}</dt>
        <dd class="gl-def">${esc(def)}</dd>
      </div>`;
    }).join('');
  }
  ...
  // GLOSSARY is populated by several modules as they load, so render after
  // they have all had their turn rather than racing them.
  addEventListener('DOMContentLoaded', () => setTimeout(() => render(''), 0));
