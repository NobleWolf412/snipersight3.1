    ...
    with RunRecorder(con, "risk", RISK_VERSION, "PORTFOLIO", "ALL") as rec:
        ...
        def settle(up_to_ts):
            ...
            for p in sorted([p for p in open_pos if p["exit_ts"] and p["exit_ts"] <= up_to_ts],
                ...
                open_pos.remove(p)
                if d not in halted and daily_pnl[d] <= -(DAILY_LOSS_LIMIT_PCT * equity):
                    halted.add(d)
                    n["KILL"] += 1
                    store.insert_fact(con, symbol="PORTFOLIO", tf="ALL", kind="risk",
        ...
        for it in intents:
                ...
                if decision != "REJECTED" and stop_dist > 0:
                    ...
                    if lev > MAX_LEVERAGE:
                        ...
                        if decision == "APPROVED":
                            decision = "REDUCED"
                        reasons.append("LEVERAGE_CAP(3x)")
            ...
            n[decision] += 1
            store.insert_fact(con, symbol=it["symbol"], tf=it["tf"], kind="risk",
        ...
        store.insert_fact(con, symbol="PORTFOLIO", tf="ALL", kind="account",
