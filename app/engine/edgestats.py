...
Three more things the numbers here do not know:
  ...
  2. Funding is not modelled ANYWHERE in this engine, and both USDT-perp symbols
     in the book settle funding 3x/day (venues.PHEMEX_PERP). Every net number
     here is therefore an OPTIMISTIC CEILING for perp symbols held overnight.
...
EDGESTATS_VERSION = "edgestats-v0.1-draft"
...
def _venue_fee_r(t: dict) -> float:
    """What this trade's fee would have been in R at the venue's REAL published
    rates, on the same two legs execsim charged.

    Rates come from venues.py and nowhere else (§6).

    Up to exec-v0.7-draft the modelled profile was coinbase-retail-v1 for every
    symbol including the USDT perps, and this column existed to size that
    mismatch. From exec-v0.8-draft execsim charges each symbol its own venue's
    rates (costs.profile_for), so for a v0.8 book this scenario should agree
    with "as recorded" to rounding. It is kept, not deleted: agreement is now a
    CHECK that the recorded fee really is the venue's fee, and the column stays
    meaningful whenever an older algo_version is passed with --algo.
    """
...
def report(con, *, algo_version: str | None = None, symbol: str | None = None,
    ...
    out = {
        ...
        "caveats": [
            ...
            "Funding is not modelled anywhere in this engine. Perp symbols held "
            "overnight pay it 3x/day, so every net number here is an OPTIMISTIC "
            "ceiling for them.",
